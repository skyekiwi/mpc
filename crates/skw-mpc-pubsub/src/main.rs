...
async fn main() -> Result<()> {
    ...
    // outgoing 
    let mut onode = MpcPubsub::new_node().await?;
    MpcPubsub::start(&mut onode)?;

    let outgoing = MpcPubsub::outgoing(&mut onode, &"test")?;
    tokio::pin!(outgoing);


    let mut inode = MpcPubsub::new_node().await?;
    MpcPubsub::start(&mut inode)?;

    let incoming = MpcPubsub::incoming(&mut inode, &"test")?;
    tokio::pin!(incoming);
