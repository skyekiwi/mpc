...
async fn main() -> Result<(), MpcNodeError> {
    ...
    let (mut main_outgoing_sender, mut main_outgoing_receiver) = mpsc::channel::<(Vec<PeerId>, PayloadHeader, mpsc::Receiver<KeyGenMessage>)>(0);
    ...
    loop {
        futures::select! {
            (peers, job_header, mut protocol_out_receiver) = main_outgoing_receiver.select_next_some() => {
                println!("Outgoing sender msg received");
                loop { 
            ...
            raw = job_assignment_receiver.next() => {
                ...
                match job_header.payload_type {
                    PayloadType::KeyGen(maybe_existing_key) => {
                        ...
                        eprintln!("sending outgoing sender");
